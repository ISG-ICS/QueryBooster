...
import defaultRuleGraphData from '../mock-api/generateRuleGraph';

const nodeWidth = 300;
const nodeHeight = 500;

const RuleGraph = NiceModal.create(({ rewriteExample }) => {
  ...
  // initial loading rule graph from server
  const getRuleGraph = (_rewriteExample) => {
    if (_rewriteExample['q0'] != "" && _rewriteExample['q1'] != "") {
     // post generateRuleGraph request to server
     axios.post('/generateRuleGraph', _rewriteExample)
     .then(function (response) {
       console.log('[/generateRuleGraph] -> response:');
       console.log(response);
       // update the states for ruleGraph
       computeNodesAndEdges(response.data);
     })
     .catch(function (error) {
       console.log('[/generateRuleGraph] -> error:');
       console.log(error);
       // mock the result
       console.log(defaultRuleGraphData);
       computeNodesAndEdges(defaultRuleGraphData);
     });
    }
  };
  
  // compute the nodes and edges for the ruleGraph
  const computeNodesAndEdges = (_ruleGraphData) => {
    // loop rules in _ruleGraph to build nodes list
    let _rules = _ruleGraphData['rules'];
    let _nodes = [];
    ...
    // loop relations in _ruleGraph to build edges list
    let _relations = _ruleGraphData['relations'];
    let _edges = [];
  ...
  React.useEffect(() => {getRuleGraph(rewriteExample);}, []);
