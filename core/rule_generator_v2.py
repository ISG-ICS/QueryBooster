...
class RuleGeneratorV2:
    ...
    def _recommendation_ast_signature(node: Optional[Node], state: Dict[str, Dict[str, str]]) -> object:
        ...
        if isinstance(node, ColumnNode):
            # Variablized columns are now ElementVariableNode — ColumnNode always has a concrete name.
    ...
    def merge_variable_list(rule: RuleV2, variable_list: List[str]) -> RuleV2:
        ...
        mapping, set_name = RuleGeneratorV2._find_next_set_variable(mapping)
        new_rule["mapping"] = mapping
    ...
    def _variable_lists_of_ast(ast: Node) -> List[List[str]]:
        ...
        def _flatten_and(node: Node) -> List[str]:
            if isinstance(node, OperatorNode) and node.name.lower() == "and":
                ...
                return names
            if isinstance(node, ElementVariableNode) and node.parent_alias is None:
    ...
    def _replace_column_in_ast(ast: Node, column: str, external_name: str) -> Node:
        """Replace every ColumnNode whose name == column (and any non-DISTINCT SELECT *) with an ElementVariableNode(external_name) in ast.

        The first column variabilized also captures bare * in plain SELECT clauses, so they share a single variable. SELECT DISTINCT * is kept separate and is only rewritten when the requested column itself is *.
        """
        ...
        for node in RuleGeneratorV2._walk(ast):
            if not isinstance(node, ColumnNode):
                continue
            if node.name == column:
                to_replace.append((node, ElementVariableNode(external_name, parent_alias=node.parent_alias, alias=node.alias)))
            elif (
                node.name == "*"
                and column != "*"
    ...
    def _replace_table_in_ast(
        ...
        """Replace every matching TableNode with an ElementVariableNode(placeholder_token) and update qualified column refs in ast.
        ...
        for node in RuleGeneratorV2._walk(ast):
            ...
            if node.name == target_value and (
                ...
                to_replace.append((node, ElementVariableNode(placeholder_token)))
    ...
    def dereplaceVars(sql: str, mapping: Dict[str, str]) -> str:
        ...
        for external_name, internal_name in mapping.items():
            for var_type in VarType:
                if internal_name.startswith(VarTypesInfo[var_type]["internalBase"]):
    ...
    def _find_next_element_variable(mapping: Dict[str, str]) -> Tuple[Dict[str, str], str]:
        """Allocate the next unused element variable in mapping and return (updated_mapping, external_name).

        Mutates mapping in place by inserting the new x? -> EV??? entry.
        """
        max_external = 0
        max_internal = 0
        for external_name, internal_name in mapping.items():
        ...
        return mapping, next_external

    @staticmethod
    def _find_next_set_variable(mapping: Dict[str, str]) -> Tuple[Dict[str, str], str]:
        """Allocate the next unused set variable in mapping and return (updated_mapping, set_name).

        Mutates mapping in place by inserting the new y? -> SV??? entry.
        """
        max_external = 0
        max_internal = 0
        for external_name, internal_name in mapping.items():
